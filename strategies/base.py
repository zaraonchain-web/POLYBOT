...
from lib.position_manager import PositionManager, Position  # ← FIXED: Added Position import
...
class BaseStrategy(ABC):
    ...
    async def execute_buy(self, side: str, current_price: float) -> bool:
        ...
        # Fetch fee rate dynamically — required for crypto markets
        fee_rate_bps = await asyncio.to_thread(
            self.bot.clob_client.get_fee_rate, token_id
        )

        self.log(f"BUY {side.upper()} @ {current_price:.4f} size={size:.2f} fee={fee_rate_bps}bps", "trade")
